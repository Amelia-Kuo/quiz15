...
public class UserServiceImpl implements UserService {
	...
	public BasicRes login(LoginReq loginReq) {
		...
		// 比對密碼
		// if條件式前有驚嘆好,等同於encoder.matches(loginreq.getPassword(), user.getPassword()) ==
		// false
	...
	public BasicRes adminLogin(AdminLoginReq adminLoginReq) {
		// 確認Email是否存在資料庫
		User user = userDao.getByEmail(adminLoginReq.getEmail());
		// 透過 email 取得一筆資料, email 不存在會得到 null
		if (user == null) {
			return new BasicRes(//
					ResCodeMessage.NOT_FOUND.getStatuscode(), //
		...
		// if條件式前有驚嘆好,等同於encoder.matches(loginreq.getPassword(), user.getPassword()) ==
		// false
		if (!encoder.matches(adminLoginReq.getPassword(), user.getPassword())) {
			return new BasicRes(//
					ResCodeMessage.PASSWORD_MISMATCH.getStatuscode(), //
					ResCodeMessage.PASSWORD_MISMATCH.getMassage());
		}
		if(!user.isAdmin()) {
			return new BasicRes(//
					ResCodeMessage.NO_ADMIN_ACCESS.getStatuscode(), //
					ResCodeMessage.NO_ADMIN_ACCESS.getMassage());
		}
